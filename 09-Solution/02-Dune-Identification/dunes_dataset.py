...
class DunesDataLoader:
    ...
    def create_splits(self):
        ...
        dunes_train, dunes_test = train_test_split(dunes_df, train_size=.7)
        not_dunes_train, not_dunes_test = train_test_split(not_dunes_df, train_size=.7)
    
        train_set = pd.concat([dunes_train, not_dunes_train], axis=0)
        test_set = pd.concat([dunes_test, not_dunes_test], axis=0)
    
        return shuffle(train_set).reset_index(), shuffle(test_set).reset_index()
